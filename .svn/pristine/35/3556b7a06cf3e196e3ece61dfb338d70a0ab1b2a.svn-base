...
public class frmCharacterExporter extends javax.swing.JFrame {

    private Vector jTableModel = new Vector(0);
    private Vector jTableHeader = new Vector(0);
    private DefaultTableModel df;
    private clsCharacter[] chars;
    private clsTemplate template;
    private boolean consolidate = false;

    /**
     * Creates new form frmCharacterExporter
     */
    public frmCharacterExporter(clsCharacter[] cs, clsTemplate t) {
    ...
    private void SortCharacters() {
        ...
        for (int x = 0; x < chars.length; x++) {
            ...
            if (selected_sort_type.isSzData) {
                ...
                clsCharacter c = (clsCharacter) chars[x].chrData.get(selected_sort_type.key);
                if (c == null) {
                    s.sortString = "";
                } else {
    ...
    private boolean ExportCharactersToTextFile(String szPath) {
        ...
        try {
            ...
            String szCharPath;
            szCharPath = szPath + File.separator + clsEngine.szSettingName + File.separator;
            ...
            for (int x = 0; x < chars.length; x++) {
                ...
                PrintWriter pw = null;
                if (consolidate) {
                    pw = new PrintWriter(new BufferedWriter(new FileWriter(szCharPath + template.GetName() + "_characters.txt", true)));
                } else {
                    szCharPath = szPath + File.separator + clsEngine.szSettingName + File.separator + chars[x].GetCharacterName() + File.separator;
                    new File(szCharPath).mkdirs();
                    pw = new PrintWriter(new BufferedWriter(new FileWriter(szCharPath + chars[x].GetCharacterName() + ".txt", false)));
                }
                ...
                for (int y = 0; y < tblData.getRowCount(); y++) {
                    ...
                    if (selected) {
                        ...
                        if (chars[x].chrData.containsKey(item.key)) {
                            ...
                            if (!ExportExtendedSheetToFile((clsExtendedSheetData) chars[x].extData.get(item.key), szCharPath + chars[x].GetCharacterName() + "_" + item.name + ".txt")) {
                                return false;
                            }
    ...
    private boolean ExportExtendedSheetToFile(clsExtendedSheetData esd, String szPath) {
    ...
    private boolean ExportCharactersToHTMLFile(String szPath) {
        ...
        try {
            ...
            for (int x = 0; x < chars.length; x++) {
                ...
                PrintWriter pw = null;
                if (consolidate) {
                    pw = new PrintWriter(new BufferedWriter(new FileWriter(szCharPath + template.GetName() + "_characters.htm", true)));
                    if (x == 0) {

                        pw.print(clsHTMLTemplate.HTML_TEMPLATE_HEADER.replace(clsHTMLTemplate.HTML_TEMPLATE_TITLE_TAG, StringEscapeUtils.escapeHtml4(clsEngine.szSettingName)).replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ID_LIST, GenerateIDList("tbl", chars.length)).replace(clsHTMLTemplate.HTML_TEMPLATE_LINK_ID_LIST, GenerateIDList("lnk", chars.length)));

                    }
                } else {
                    szCharPath = szPath + File.separator + clsEngine.szSettingName + File.separator + chars[x].GetCharacterName() + File.separator;
                    new File(szCharPath).mkdirs();
                    pw = new PrintWriter(new BufferedWriter(new FileWriter(szCharPath + chars[x].GetCharacterName() + ".htm", false)));
                ...
                pw.print(clsHTMLTemplate.HTML_TEMPLATE_TABLE_HEADER_ROW.replace(clsHTMLTemplate.HTML_TEMPLATE_CHARACTER_NAME_TAG, StringEscapeUtils.escapeHtml4(chars[x].GetCharacterName())).replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ID, "tbl" + x).replace(clsHTMLTemplate.HTML_TEMPLATE_EXPAND_LINK_ID, "lnk" + x));
                ...
                for (int y = 0; y < tblData.getRowCount(); y++) {
                    ...
                    if (selected && !item.key.equals(clsConstants.CHARACTER_NAME)) {

                        if (chars[x].chrData.containsKey(item.key)) {

                            pw.print(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ROW.replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_NAME, StringEscapeUtils.escapeHtml4(item.name)).replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_DATA, StringEscapeUtils.escapeHtml4(chars[x].chrData.get(item.key).toString())));

                        } else if (chars[x].szData.containsKey(item.key)) {

                            pw.print(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ROW.replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_NAME, StringEscapeUtils.escapeHtml4(item.name)).replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_DATA, StringEscapeUtils.escapeHtml4(chars[x].szData.get(item.key).toString()).replace("\n", "<br>")));

                        } else if (chars[x].extData.containsKey(item.key)) {

                            pw.print(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ROW.replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_NAME, StringEscapeUtils.escapeHtml4(item.name)).replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_DATA, ExpandedSheetToHTMLTable((clsExtendedSheetData) chars[x].extData.get(item.key))));
                            ...
                            pw.print(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ROW.replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_NAME, StringEscapeUtils.escapeHtml4(item.name)).replace(clsHTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_DATA, "<img src='" + szImgPath + "'/>"));
                ...
                //Add table footer.
                pw.print(clsHTMLTemplate.HTML_TEMPLATE_FOOTER_ROW);
                if (!consolidate || (consolidate && x == chars.length - 1)) {

                    Calendar currentDate = Calendar.getInstance();
                    SimpleDateFormat formatter = new SimpleDateFormat("HH:mm:ss d MMM yyyy");

                    pw.print(clsHTMLTemplate.HTML_TEMPLATE_FOOTER.replace(clsHTMLTemplate.HTML_TEMPLATE_FOOTER_TEXT, "Generated by </i><a href='" + clsEngine.szHomePage + "'>XEED " + clsEngine.szVersion + " Build " + clsEngine.lngBuild + "." + clsEngine.GetXEEDCRC32() + "</a><i> at " + formatter.format(currentDate.getTime())));
                }
    ...
    private String ExpandedSheetToHTMLTable(clsExtendedSheetData esd) {
    ...
    private PdfPTable ExpandedSheetToPDFTable(clsExtendedSheetData esd, Font f) {
    ...
    private boolean ExportCharactersToPDF(String szPath) {
        ...
        Anchor anchor = new Anchor("XEED " + clsEngine.szVersion + " Build " + clsEngine.lngBuild + "." + clsEngine.GetXEEDCRC32(), footerFont_link);
        anchor.setReference(clsEngine.szHomePage);
            ...
            for (int x = 0; x < chars.length; x++) {
                ...
                if (consolidate) {
                    ...
                    if (x == 0) {
                        ...
                        PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(szCharPath + template.GetName() + "_characters.pdf"));     //start outputting.
                        ...
                        Paragraph title = new Paragraph(clsEngine.szSettingName, titleFont);     //Add title
                    ...
                    document = new Document(PageSize.A4, 36, 36, 54, 54);                                                                        //open the document
                    PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(szCharPath + chars[x].GetCharacterName() + ".pdf"));     //start outputting.
                    HeaderFooter event = new HeaderFooter();
                    event.setFooter(footer);
                    writer.setBoxSize("art", new Rectangle(36, 30, 559, 800));
                    writer.setPageEvent(event);
                ...
                for (int y = 0; y < tblData.getRowCount(); y++) {
                    ...
                    if (selected && !item.key.equals(clsConstants.CHARACTER_NAME)) {
                        ...
                        if (chars[x].chrData.containsKey(item.key)) {
                            ...
                            cell = new PdfPCell(ExpandedSheetToPDFTable((clsExtendedSheetData) chars[x].extData.get(item.key), subTableBodyFont));
                            cell.setBorder(PdfPCell.NO_BORDER);
                            cell.setPadding(2f);
    ...
    private void btnExportActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExportActionPerformed
        ...
        if (chars.length == 0) {
            JOptionPane.showMessageDialog(null, "No characters selected. Please select characters in the main window and restart the character exporter. ", "Attention", JOptionPane.INFORMATION_MESSAGE);
            clsEngine.hwndCharacterExporter = null;
            dispose();
            return;
        }
        ...
        if (ret) {
            JOptionPane.showMessageDialog(null, "Characters succesfully exported!", "Success!", JOptionPane.INFORMATION_MESSAGE);
        } else {
    ...
    class HeaderFooter extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_RIGHT, new Phrase(Integer.toString(writer.getPageNumber())), rect.getRight(), rect.getTop(), 0);
