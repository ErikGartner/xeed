...
public class frmCharacter extends javax.swing.JFrame {
    ...
    private pnlCharacter topPanel;
    public clsCharacter character;

    public frmCharacter(pnlCharacter panel, clsCharacter c) {
    ...
    public void ReloadItems(boolean RelationCharacters, boolean Groups, boolean boolItems, boolean Relations) {
        ...
        if (boolItems) {
            ...
            for (int x = 0; x < topPanel.items.size(); x++) {
                ...
                if (topPanel.items.get(x).getClass() == pnlOffspring.class) {

                    ((pnlOffspring) topPanel.items.get(x)).LoadOffspring();

                }
                if (topPanel.items.get(x).getClass() == pnlParentRow.class) {

                    ((pnlParentRow) topPanel.items.get(x)).LoadData();
