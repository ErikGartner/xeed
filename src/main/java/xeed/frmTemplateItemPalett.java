...
import xeed.frmTemplateCreator.TreeItem;
 ...
 * @author Erik
 */
public class frmTemplateItemPalett extends javax.swing.JFrame {

    DefaultMutableTreeNode editingNode;
    TreeItem editingItem;
    frmTemplateCreator form;
    boolean DiscardInsteadOfDeleting;

    public frmTemplateItemPalett(frmTemplateCreator f, DefaultMutableTreeNode e, boolean discard) {
    ...
    private void btnOKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOKActionPerformed
        ...
        if (txtName.getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "The name property cannot be empty.", "Missing data", JOptionPane.WARNING_MESSAGE);
            return;
        }

        if (((ComboItem) comboType.getSelectedItem()).use_data) {
            if (txtData.getText().isEmpty()) {
                JOptionPane.showMessageDialog(null, "The data property cannot be empty.", "Missing data", JOptionPane.WARNING_MESSAGE);
                return;
            }
            String[] values = txtData.getText().split("\n");
            for (int x = 0; x < values.length; x++) {
                if (values[x].isEmpty()) {
                    JOptionPane.showMessageDialog(null, "The data property shouldn't and can't contain any empty rows.", "Missing data", JOptionPane.WARNING_MESSAGE);
                    return;
                }
            }
        ...
        if (((ComboItem) comboType.getSelectedItem()).Type == 6) {            //namerow
            if (form.nameRow != null) {
                JOptionPane.showMessageDialog(null, "A name row already exists. There can be only one!", "Invalid data!", JOptionPane.WARNING_MESSAGE);
                return;
            } else {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        JOptionPane.showMessageDialog(null, ((ComboItem) comboType.getSelectedItem()).szHelp, "Help: " + ((ComboItem) comboType.getSelectedItem()).szName, JOptionPane.INFORMATION_MESSAGE);
