 ...
 * @author Erik
 */
public final class pnlOffspring extends javax.swing.JPanel {

    public clsCharacter character;
    private DefaultMutableTreeNode topNode = null;

    public pnlOffspring(clsCharacter c, String name) {
        character = c;
        initComponents();
    ...
    private void _printchildren(clsCharacter c, DefaultMutableTreeNode n) {
        ...
        for (int x = 0; x < clsEngine.charDB.size(); x++) {
            ...
            if (clsEngine.charDB.get(x).chrData.containsValue(c)) {
                DefaultMutableTreeNode child = new DefaultMutableTreeNode(clsEngine.charDB.get(x));
                n.add(child);
                _printchildren(clsEngine.charDB.get(x), child);
            }
    ...
    private void treeOffspringMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_treeOffspringMouseClicked
        ...
        clsCharacter chr = (clsCharacter) nodeInfo;
