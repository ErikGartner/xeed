...
public class CharacterExporterForm extends javax.swing.JFrame {
        ...
        try {
            ...
            for (int x = 0; x < chars.length; x++) {
                ...
                if (consolidate) {
                    ...
                    szCharPath = szPath + File.separator + XEED.szSettingName + File.separator + chars[x].GetCharacterName() + File.separator;
                    new File(szCharPath).mkdirs();
                ...
                for (int y = 0; y < tblData.getRowCount(); y++) {
                    ...
                    if (selected) {
                        ...
                        if (chars[x].chrData.containsKey(item.key)) {
                            ...
                            if (!ExportExtendedSheetToFile((ExtendedSheetData) chars[x].extData.get(item.key), szCharPath + chars[x].GetCharacterName() + "_" + item.name + ".txt")) {
                                return false;
                            }
            ...
            for (int x = 0; x < chars.length; x++) {
                ...
                if (consolidate) {
                    ...
                    if (x == 0) {
                        ...
                        pw.print(HTMLTemplate.HTML_TEMPLATE_HEADER.replace(HTMLTemplate.HTML_TEMPLATE_TITLE_TAG, StringEscapeUtils.escapeHtml4(XEED.szSettingName)).replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ID_LIST, GenerateIDList("tbl", chars.length)).replace(HTMLTemplate.HTML_TEMPLATE_LINK_ID_LIST, GenerateIDList("lnk", chars.length)));
                ...
                pw.print(HTMLTemplate.HTML_TEMPLATE_TABLE_HEADER_ROW.replace(HTMLTemplate.HTML_TEMPLATE_CHARACTER_NAME_TAG, StringEscapeUtils.escapeHtml4(chars[x].GetCharacterName())).replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ID, "tbl" + x).replace(HTMLTemplate.HTML_TEMPLATE_EXPAND_LINK_ID, "lnk" + x));
                ...
                for (int y = 0; y < tblData.getRowCount(); y++) {
                    ...
                    if (selected && !item.key.equals(Constants.CHARACTER_NAME)) {
                        ...
                        if (chars[x].chrData.containsKey(item.key)) {
                            ...
                            pw.print(HTMLTemplate.HTML_TEMPLATE_TABLE_ROW.replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_NAME, StringEscapeUtils.escapeHtml4(item.name)).replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_DATA, StringEscapeUtils.escapeHtml4(chars[x].chrData.get(item.key).toString())));

                        } else if (chars[x].szData.containsKey(item.key)) {

                            pw.print(HTMLTemplate.HTML_TEMPLATE_TABLE_ROW.replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_NAME, StringEscapeUtils.escapeHtml4(item.name)).replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_DATA, StringEscapeUtils.escapeHtml4(chars[x].szData.get(item.key).toString()).replace("\n", "<br>")));

                        } else if (chars[x].extData.containsKey(item.key)) {

                            pw.print(HTMLTemplate.HTML_TEMPLATE_TABLE_ROW.replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_NAME, StringEscapeUtils.escapeHtml4(item.name)).replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_DATA, ExpandedSheetToHTMLTable((ExtendedSheetData) chars[x].extData.get(item.key))));
                            ...
                            pw.print(HTMLTemplate.HTML_TEMPLATE_TABLE_ROW.replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_NAME, StringEscapeUtils.escapeHtml4(item.name)).replace(HTMLTemplate.HTML_TEMPLATE_TABLE_ITEM_DATA, "<img src='" + szImgPath + "'/>"));
                ...
                if (!consolidate || (consolidate && x == chars.length - 1)) {
                    ...
                    pw.print(HTMLTemplate.HTML_TEMPLATE_FOOTER.replace(HTMLTemplate.HTML_TEMPLATE_FOOTER_TEXT, "Generated by </i><a href='" + XEED.szHomePage + "'>XEED " + XEED.szVersion + " Build " + XEED.lngBuild + "." + XEED.GetXEEDCRC32() + "</a><i> at " + formatter.format(currentDate.getTime())));
                }
    ...
    private boolean ExportCharactersToPDF(String szPath) {
        ...
        Anchor anchor = new Anchor("XEED " + XEED.szVersion + " Build " + XEED.lngBuild + "." + XEED.GetXEEDCRC32(), footerFont_link);
                ...
                for (int y = 0; y < tblData.getRowCount(); y++) {
                    ...
                    if (selected && !item.key.equals(Constants.CHARACTER_NAME)) {
                        ...
                        if (chars[x].chrData.containsKey(item.key)) {
                            ...
                            cell = new PdfPCell(ExpandedSheetToPDFTable((ExtendedSheetData) chars[x].extData.get(item.key), subTableBodyFont));
                            cell.setBorder(PdfPCell.NO_BORDER);
                            cell.setPadding(2f);
