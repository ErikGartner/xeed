...
public final class frmGraph extends javax.swing.JFrame {
    ...
    private pnlRelationGraph RelationPanel = null;
    private pnlGenealogyGraph GenealogyPanel = null;
    /*
    ...
    public void LoadTemplates() {
        ...
        for (int x = 0; x < clsEngine.templateDB.size(); x++) {
            comboGenTemplate.addItem(new TemplateOption(clsEngine.templateDB.get(x)));
        }
    ...
    public void LoadCharactersAndGroups() {
        ...
        ArrayList<clsCharacter> selectedrelchars = new ArrayList(0);
        for (int x = 0; x < jTableModelRelChar.size(); x++) {
            Vector o = (Vector) jTableModelRelChar.get(x);
            if ((Boolean) o.get(0) == true) {
                selectedrelchars.add((clsCharacter) o.get(1));
            }
        }

        ArrayList<clsGroup> selectedgroups = new ArrayList(0);
        for (int x = 0; x < jTableModelRelGroup.size(); x++) {
            Vector o = (Vector) jTableModelRelGroup.get(x);
            if ((Boolean) o.get(0) == true) {
                selectedgroups.add((clsGroup) o.get(1));
            }
        }

        ArrayList<clsCharacter> selectedgenchars = new ArrayList(0);
        for (int x = 0; x < jTableModelGenChar.size(); x++) {
            Vector o = (Vector) jTableModelGenChar.get(x);
            if ((Boolean) o.get(0) == true) {
                selectedgenchars.add((clsCharacter) o.get(1));
            }
        }
        ...
        for (int x = 0; x < clsEngine.charDB.size(); x++) {
            ...
            o2.add(selectedrelchars.contains(clsEngine.charDB.get(x)));
            o2.add(clsEngine.charDB.get(x));
            jTableModelRelChar.add(o2);
        }

        for (int x = 0; x < clsEngine.groupDB.size(); x++) {
            Vector o = new Vector(0);
            o.add(selectedgroups.contains(clsEngine.charDB.get(x)));
        ...
        for (int x = 0; x < clsEngine.charDB.size(); x++) {
            Vector o2 = new Vector(0);
            o2.add(jTableModelGenChar.contains(clsEngine.charDB.get(x)));
            o2.add(clsEngine.charDB.get(x));
    ...
    private void LoadRelationGraph() {
        ...
        RelationPanel = new pnlRelationGraph();
    ...
    private void LoadGenealogyGraph() {
        ...
        clsCharacter[] cs = new clsCharacter[selectedgenchars.size()];
        selectedgenchars.toArray(cs);

        clsTemplate[] ts = new clsTemplate[comboGenTemplate.getItemCount()];
        ...
        if (cs.length > 0 && ts.length > 0) {
            ...
            GenealogyPanel = new pnlGenealogyGraph();
    ...
    private void UpdateRelations() {
        ...
        ArrayList<clsCharacter> tmp = new ArrayList(0);
        for (int x = 0; x < jTableModelRelChar.size(); x++) {
            Vector o = (Vector) jTableModelRelChar.get(x);
            if ((Boolean) o.get(0) == true) {
                tmp.add((clsCharacter) o.get(1));
            }
        }

        ArrayList<clsGroup> tmp2 = new ArrayList(0);
        for (int x = 0; x < jTableModelRelGroup.size(); x++) {
            Vector o = (Vector) jTableModelRelGroup.get(x);
            if ((Boolean) o.get(0) == true) {

                if (chkUseMembers.isSelected()) {
                    long[] members = ((clsGroup) o.get(1)).GetMemebers();
                    for (int y = 0; y < members.length; y++) {
                        clsCharacter member = clsEngine.GetCharacterByID(members[y]);
                        if (!tmp.contains(member)) {
                            tmp.add(member);
                        }
                    }
                } else {
                    tmp2.add((clsGroup) o.get(1));
                }

            }
        }
        if (tmp.size() > 0) {
            c = new clsCharacter[tmp.size()];
            tmp.toArray(c);
        } else {
            c = null;
        }
        if (tmp2.size() > 0) {
            g = new clsGroup[tmp2.size()];
            tmp2.toArray(g);
        } else {
            g = null;
        }
    ...
    private void initComponents() {
        ...
        tblGenChars.setModel(new javax.swing.table.DefaultTableModel(
            jTableModelGenChar,
            jTableHeaderGenChar
        ) {
        ...
        jPanel3Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel3Layout.createSequentialGroup()
                ...
                .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(lblGenColor, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addComponent(comboGenTemplate, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addComponent(comboGenPicture, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addGroup(jPanel3Layout.createSequentialGroup()
                        .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(comboGenExtra, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                .addContainerGap())
        );
        jPanel3Layout.setVerticalGroup(
            ...
            .addGroup(jPanel3Layout.createSequentialGroup()
                ...
                .addComponent(comboGenTemplate, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addComponent(jLabel5)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(lblGenColor, javax.swing.GroupLayout.PREFERRED_SIZE, 22, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addComponent(jLabel7)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(comboGenPicture, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addComponent(jLabel8)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(comboGenExtra, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
        ...
        tblRelChars.setModel(new javax.swing.table.DefaultTableModel(
            jTableModelRelChar,
            jTableHeaderRelChar
        ) {
            Class[] types = new Class [] {
        ...
        tblRelGroups.setModel(new javax.swing.table.DefaultTableModel(
            jTableModelRelGroup,
            jTableHeaderRelGroup
        ) {
            Class[] types = new Class [] {
                java.lang.Boolean.class, Object.class
            };
        ...
        chkUseMembers.setToolTipText("Instead of displaying the group as an object its members are. This is useful if you want to view the relations within the group fast by selecting the group.");
        ...
        jPanel2Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(chkUseMembers, javax.swing.GroupLayout.DEFAULT_SIZE, 182, Short.MAX_VALUE)
    ...
    class TemplateOption {
        ...
        public TemplateOption(clsTemplate t) {
            this.t = t;
        }
