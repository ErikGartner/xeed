 ...
 * @author Erik
 */
public class pnlRelationGraph extends javax.swing.JPanel {
    ...
    public boolean LoadData(clsCharacter[] chars, clsGroup[] grps) {
    ...
    public void CalculatePosAndDrawDots(Graphics2D g) {
        ...
        for (int x = 0; x < Dots.length; x++) {
            if (Dots[x].type == 0) {
                ReservedBorders_sides = Math.max(ReservedBorders_sides, fm.stringWidth(((clsCharacter) Dots[x].entity).GetCharacterName()) + DotSize);
            } else if (Dots[x].type == 1) {
                ReservedBorders_sides = Math.max(ReservedBorders_sides, fm.stringWidth(((clsGroup) Dots[x].entity).szName) + DotSize);
            }
        }
    ...
    public void DrawNames(Graphics2D g) {
        ...
        for (int x = 0; x < Dots.length; x++) {
            ...
            String szName = "";
            if (Dots[x].type == 0) {
                szName = ((clsCharacter) Dots[x].entity).GetCharacterName();
            } else if (Dots[x].type == 1) {
                szName = ((clsGroup) Dots[x].entity).szName;
            }
    ...
    private void DrawRelations(Graphics2D g) {
        ...
        for (int x = 0; x < Dots.length; x++) {
            for (int y = x + 1; y < Dots.length; y++) {
                ...
                String szRelYtoX = "";
                if (Dots[x].type == 0) {
                    szRelXtoY = ((clsCharacter) Dots[x].entity).GetRelation(Dots[y].lngID, Dots[y].type);
                } else {
                    szRelXtoY = ((clsGroup) Dots[x].entity).GetRelation(Dots[y].lngID, Dots[y].type);
                }
                if (Dots[y].type == 0) {
                    szRelYtoX = ((clsCharacter) Dots[y].entity).GetRelation(Dots[x].lngID, Dots[x].type);
                } else {
                    szRelYtoX = ((clsGroup) Dots[y].entity).GetRelation(Dots[x].lngID, Dots[x].type);
                }
                ...
                if (!szRelXtoY.isEmpty() || !szRelYtoX.isEmpty()) {
                    ...
                    if (Dots[x].x < Dots[y].x + 2 && Dots[x].x > Dots[y].x - 2) {
                        ...
                        if (evt.getX() <= Dots[x].x + 8 && evt.getX() >= Dots[x].x - 8 && evt.getY() < Math.max(Dots[x].y, Dots[y].y) && evt.getY() > Math.min(Dots[x].y, Dots[y].y)) {
                            ...
                            clsEngine.hwndRelations.LoadSpecificRelation(Dots[x].entity, Dots[y].entity);
                            return;
                        }
